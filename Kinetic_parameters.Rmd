...
La librería drc es para modelos no lineales de dosis-respuesta La
librería renz es solamente para cinetica enzimatica La librería
tidyverse y dplyr es de uso común para múltiples funciones Y la librería
ggplot2 es una de las más usadas para visualizaci´no de datos y
generación de plots
...
Ahora llamamos a las librerías instaladas y creamos el dataframe Se
puede hacer manualmente en R, o extraerlo de un archivo delimitado por
comas (.csv), en la pestaña de Enviroment, dando clicl al recuadro
Import Dataset.

Como el dataframe se genera el formato "ancho", usando pivot_longer() lo
pasamos a formato "largo", para que R pueda graficar correctamente
nuestros datos.
...
Calculamos la media y la desviación estándar para poder colocarlas en
nuestro gráfico de Michaelis-Menten
...
Ahora ajustamos el modelo de Michaelis-Menten usando la función de drm()
de la librería DRC, que es la función para dosis-response. Antes de
generar el modelo, debemos crear predicciones de las constantes
cinéticas.
...
La librería RENZ nos ofrece la función dir.MM(), que es específica para
el modelo de Michaelis-Menten. Nos genera automáticamente un gráfico
simple de nuestros datos, la Km y velocidad máxima. Sin embargo, si
nuestros datos son muy pequeños, las constantes no mostrarán todos los
dígitos decimales.
...
Ggplot es el paquete por excelencia para generar gráficos Para que se
grafique con una curva de ajuste (línea de tendencia en Excel) se debe
generar otro dataframe de estos datos y después incorporarlos en el
gráfico.
...
También se puede generar un plot más simple, sin necesidad de una
paquetería como ggplot, con el siguiente código.
